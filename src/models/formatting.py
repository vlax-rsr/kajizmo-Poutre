...
def afficher_choix(poutre, compositions: list[tuple[int, int]]) -> list[str]:
    ...
    text_choix = [
        "\n" + "=" * LARGEUR_RACCORD,
        f"{'COMPOSITION DE L ARMATION RÉELLE SÉLECTIONNÉE':^{LARGEUR_RACCORD}}",
        "=" * LARGEUR_RACCORD
    ]
...
def afficher_result_trans(poutre, phi_t: int, nb_brins: int) -> list[str]:
    ...
    text_trans.extend([
        "\n" + "=" * LARGEUR_RACCORD,
        f"{'RÉSULTATS DES ARMATURES TRANSVERSALES':^{LARGEUR_RACCORD}}",
        "=" * LARGEUR_RACCORD,
        "\nCARACTÉRISTIQUES DU COURS DE CADRE",
        f"  Diamètre de l'armature choisi      : HA{res_trans.get('phi_t', phi_t)}",
        f"  Nombre de brins verticaux (n)      : {res_trans.get('nb_brin', nb_brins)} brins",
...
def afficher_result_fleche(poutre) -> list[str]:
    ...
    text_fleche = [
        "=" * LARGEUR_RACCORD,
        f"{'VÉRIFICATION EXPÉRIMENTALE DE LA FLÈCHE':^{LARGEUR_RACCORD}}",
        "=" * LARGEUR_RACCORD
    ]
    ...
    text_fleche.extend([
        f"  Flèche maximale admissible (f_lim)          : {res_fleche['f_lim_cm']:7.2f} cm",
        f"  Flèche maximale calculée (f)                : {res_fleche['f_cm']:7.2f} cm",
    ])
